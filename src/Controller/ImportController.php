...
class ImportController extends AbstractController
{
    ...
    /**
     * @Route("/test-email/{adresse}", name="test_email")
     */
    public function testEmail($adresse)
    {
        $users = $this->getDoctrine()->getManager()->getRepository(Doctor::class)->findAll();
        $user = $users[0];
        $subject = 'Welcome to DocBooking!';
        $message = $this->renderView(
            'emails/registration_doctor.html.twig',
            [
                'title' => $user->getTitle()??'Test',
